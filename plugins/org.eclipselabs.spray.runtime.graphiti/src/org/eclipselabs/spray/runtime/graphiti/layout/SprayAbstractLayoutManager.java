...
public abstract class SprayAbstractLayoutManager implements ISprayLayoutManager {
	...
	static public void fixOffset(ContainerShape containerShape) {
		System.out.println("BEGIN FixOffset: "
				+ SprayLayoutService.getId(containerShape));
	...
	static protected void fixOffsetForShape(ContainerShape containerShape) {
		...
		System.out.println("BEGIN FixOffset for shape: "
				+ SprayLayoutService.getId(containerShape) + " active "
				+ active);
		if (active) {
			if (!containerShape.getContainer().isActive()) {
				...
				if (parent != null) {
					GraphicsAlgorithm containerGa = containerShape
							.getGraphicsAlgorithm();
					System.out.println("    fixing shape "
							+ SprayLayoutService.getId(containerShape)
							+ " old " + containerGa.getX() + ", "
							+ containerGa.getY() + " offset " + xOffset + ", "
							+ yOffset);
					Graphiti.getGaService().setLocation(containerGa,
							containerGa.getX() + xOffset,
							containerGa.getY() + yOffset);
					System.out.println("    new: " + containerGa.getX() + ", "
							+ containerGa.getY());
				}
			}
		}
	...
	static protected void OLDfixOffsetForShape(ContainerShape containerShape) {
		...
		for (Shape child : containerShape.getChildren()) {
			...
			if (childGa instanceof Polyline) {
				...
				// Graphiti.getGaService().setLocation(childGa, childGa.getX() +
				// xOffset, childGa.getY() + yOffset);
			} else {
				System.out.println("    fixing child "
						+ SprayLayoutService.getId(child) + " old "
						+ childGa.getX() + ", " + childGa.getY() + " offset "
						+ xOffset + ", " + yOffset);
				Graphiti.getGaService().setLocation(childGa,
						childGa.getX() + xOffset, childGa.getY() + yOffset);
				System.out.println("    new: " + childGa.getX() + ", "
						+ childGa.getY());
			}
		}
		System.out.println("END FixOffset for shape");
	...
	public int getMargin() {
		return GraphitiProperties.getIntValue(shape,
				SprayLayoutService.LAYOUT_MARGIN);
	}
	...
	public int getSpacing() {
		return GraphitiProperties.getIntValue(shape,
				SprayLayoutService.LAYOUT_SPACING);
	}
	...
	public void setSpacing(int spacing) {
		GraphitiProperties.set(shape, SprayLayoutService.LAYOUT_SPACING,
				spacing);
	}

	@Override
	public SprayAlignment getAlignment() {
		return GraphitiProperties.getSprayAlignmentValue(shape,
				SprayLayoutService.LAYOUT_ALIGNMENT);
	}

	@Override
	public void setAlignment(SprayAlignment alignment) {
		GraphitiProperties.set(shape, SprayLayoutService.LAYOUT_ALIGNMENT,
				alignment);
	}
	...
	static public void print(Shape shape, int level) {
		...
		System.out.println(SprayLayoutService.getId(shape) + " ==> "
				+ ga.getClass() + " X : " + ga.getX() + " Y : " + ga.getY()
				+ " W : " + ga.getWidth() + " H : " + ga.getHeight());
	...
	static public void printLayout(Shape expressionShape, int level) {
		...
		ISprayLayoutManager mgr = SprayLayoutService
				.getLayoutManager(expressionShape);
		SprayLayoutData data = SprayLayoutService
				.getLayoutData(expressionShape);
		System.out.println(SprayLayoutService.getId(expressionShape) + " ==> "
				+ ga.getClass() + " Layout : " + mgr.getClass().getSimpleName()
				+ " margin : " + mgr.getMargin() + " spacing: "
				+ mgr.getSpacing() + " MinW : " + data.getMinimumWidth()
				+ " MinH : " + data.getMinimumHeight());
	...
	static public void setDefaultTextAttributes(Diagram diagram,
			AbstractText ret, String value) {
