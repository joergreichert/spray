...
class StyleJvmModelInferrer extends AbstractModelInferrer {
	...
	def void infer(Style element, IAcceptor<JvmDeclaredType> acceptor, boolean isPrelinkingPhase) {
	...
	def void infer(Gradient element, IAcceptor<JvmDeclaredType> acceptor, boolean isPrelinkingPhase) {
		val typeProvider = typeProviderFactory.createTypeProvider
		var existingClass = typeProvider.findTypeByName(element.packageName + "." + element.className)
		if(existingClass == null) {
			acceptor.accept(element.toClass(converter.toQualifiedName(element.packageName + "." + element.className)) [
