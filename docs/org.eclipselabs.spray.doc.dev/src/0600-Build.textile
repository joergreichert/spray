...
The common Maven settings are located at @devtools/org.eclipselabs.spray.devenv/settings.xml@. This settings file configures the remote repositories that Spray needs (not everything is available through Maven Central) the server settings for deployment onto the Cloudbees Repository.
...
See the "Maven Password Encryption Guide":http://maven.apache.org/guides/mini/guide-encryption.html for more details.
...
bc. mvn clean install -Dlocal-build 
...
To learn more about Sonar (now called SonarQube) have a look at the "offical site":http://www.sonarsource.org/
...
To learn more about JaCoCo see "here":http://www.eclemma.org/
...
The "artifacts":https://spray.ci.cloudbees.com/job/spray-ci-targetplatform/lastSuccessfulBuild/artifact/releng/org.eclipselabs.spray.repository/target/repository/ of this job is the P2 repository that is used by the @spray-ci-build@. 
...
The build configuration is based on Tom Schindl's "DistroBuilder":https://github.com/tomsontom/distrobuilder
...
A build on the Cloudbees Jenkins service cannot start an UI. To execute the UI tests anyway Xvnc server is started within the build. Additionally the metacity window manager is started as a pre-build step with Shell execution. The required hints to solve this issue were found in the blog entry "How to run UI tests on hudson.eclipse.org server (on a server without a window manager activated by default)":http://www.bonitasoft.org/blog/eclipse/how-to-run-ui-tests-on-hudson-eclipse-org-server-on-a-server-without-a-window-manager-activated-by-default/ 
...
* Go to the "Downloads page":http://code.google.com/a/eclipselabs.org/p/spray/downloads/list and remove the "Featured" tag from the artifacts of the older release, then add the "Deprecated" tag.
