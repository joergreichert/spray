...
In the Preferences of Eclipse you should see a new selected Java/CodeStyle/Formatter called "spray_eclipse_formatter".

h3. Add Spray task repository to Eclipse 

First you have to install Google Code Task Repository manually.
* For this go to Help > Install New Software > Add 
** Name: Mylyn Google Location
** http://knittig.de/googlecode-mylyn-connector/update/
* Select the new Source and Install the complete NightlyBuilds.

Then you can configure the task repository.
...
** enter your Google Account credentials (now use as password your Google Mail password, not the Google Code Password) > Validate Settings > Finish
!images/mylyn_1.png!
* Add New Query
...
We are targeting for Eclipse Kepler. A "target platform definition":http://help.eclipse.org/kepler/index.jsp?topic=/org.eclipse.pde.doc.user/concepts/target.htm 
