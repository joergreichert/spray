...
*Update:* With the current Spray snapshot version all projects created via Spray project wizard will contain an initial plugin.xml as well as a mechanism that updates the plugin.xml in project root after each save in the project.   
...
h2. Extending the 5 Minute Tutorial
