...
<xsl:stylesheet version="1.0"
	...
	xmlns="http://www.w3.org/1999/XSL/Format" xmlns:old="SprayUserGuide.xml"
	...
	<xsl:template
		match="//xslt:flow/xslt:block/xslt:block[text()='Spray User Guide']">
		<xsl:comment>
			Beginning with Logo
		</xsl:comment>
	...
	<xsl:template match="//xslt:inline[starts-with(text(),'http')]">
		...
		<xsl:otherwise>
				<inline text-decoration="underline" color="blue" keep-together="auto">
			<xsl:value-of select="."></xsl:value-of>
		</inline>
		</xsl:otherwise>
		</xsl:choose>
