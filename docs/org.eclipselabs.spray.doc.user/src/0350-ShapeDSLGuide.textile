...
The Shape DSL is a geometric description for the visual representation of a figure. A figure mostly consists of at least one or more geometric forms which are implemented with the Graphiti framework. If a more complex figure is needed basic forms can be nested together.

h2. Shape examples

bc.. shape Usecase (String textline) style org.eclipselabs.spray.styles.NoTransparency 
	{
...
p. 
!images/shapes_bpmn_eventmail.png!
...
For every defined shape a Java class will be generated. The interface ISprayShape provides one method
which will be defined for every class.

bc. 
public ContainerShape getShape(Diagram diagram);

The implemented method @getShape()@ returns a ContainerShape (@org.eclipse.graphiti.mm.pictograms.ContainerShape@) which is comprised of all elements of the defined figure/shape.
...
If elements are at the same level like an ellipse and a text they will be defined among each other, so shapes can be multiple nested. 

bc. shape Usecase { ellipse {} text {} }

Every figure have the attributes @size-min@, @size-max@, @stretching@ and @proportional@ that can be set for each definition. The @size-min@ / @size-max@ is the minimum/maximum bound of a figure to assure to display them correctly. The attributes @streching@ and @proportional@ affects the resize abilities of a shape. @streching@ has an attribute for @horizinal@ and @vertical@ stretching of the figure. @proportional@ handles whether the resize proportion between height and width is kept or not.
...
Every shape has the ability to place different types of anchors with the keyword @anchor@. An anchor is specified at the first level of a shape and gives the shape the possibility to place a connection. There are four options to place an anchor. The first 2 options are dynamic and can be set with a keyword @anchor = center@ or @anchor = corners@. It does not matter what kind of shape is chosen, the anchor will be placed in the center or at the corners where the shape is drawn onto. If an @ellipse@ is defined, this shape will be drawn on an invisible rectangle thus the corners of the @anchor@ are the one from the rectangle.

There are two more options to define an @anchor@: a relative and a fixed one. A relative anchor type looks like this:
...
Every position entry represent an anchor point for a connection. In this case the @anchor@ has an attribute @position@ with two values @xoffset@ and @yoffset@. The @xoffset@/ @yoffset@ is the distance from the upper left corner to given value.
...
The keyword @text@ represents a shape element which renders a string.

bc. text { ... }

A text field has one obligatory and one optional attribute. The first attribute is the alignment (@align@), which can have the value @horizontal@ or @vertical@. For both values enumeration values are referenced. The first value can be @left@,@right@ or @center@ and the second one @top@, @middle@ or @bottom@. The optional attribute can set the @"text"@ value of the shape.
...
This shape needs a pair of @point@ which define the start point and the end point of a line. Each point has their @x=@/ @y=@ coordinates.
...
A polyline definition requires a set of @point@ to specify every start and end of a route. Each point has their @x=@/ @y=@ coordinates.
...
All 2d shape definitions have two obligatory attributes @size@ and @position@. The @size@ defines the @width@ and @height@ of a 2d shape. The @position@ defines the @x@ and @y@ coordinates where the figure is displayed.

h3. Ellipse

The keyword @ellipse@ represents an ellipse shape element.
...
A polygon is a shape with n sides. This figure is bounded by a closed path, this means the start point is connected to the end point. Every side is defined by two @point@ with there @x@ and @y@ coordinates.
