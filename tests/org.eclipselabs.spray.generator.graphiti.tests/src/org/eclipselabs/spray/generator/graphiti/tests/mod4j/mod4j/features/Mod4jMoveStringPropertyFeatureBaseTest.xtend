...
class Mod4jMoveStringPropertyFeatureBaseTest extends AbstractSprayGeneratorTest {

    @Test
    def void test() {
       val fsa = triggerGenerator("mod4j/mod4j-busmod.spray")
       val keyClass = "DEFAULT_OUTPUTorg/eclipselabs/spray/examples/mod4j/features/Mod4jMoveStringPropertyFeatureBase.java"
    ...
    def private expectedContent() '''
        ...
        public abstract class Mod4jMoveStringPropertyFeatureBase extends DefaultMoveShapeFeature {
        
            SprayLayoutManager layoutManager; 
        
            public Mod4jMoveStringPropertyFeatureBase(final IFeatureProvider fp) {
                super(fp);
            ...
            public boolean canMoveShape(IMoveShapeContext context) {
                ...
                if ( getBusinessObjectForPictogramElement(sourceShape.getContainer())instanceof UniqueRule) {
                    return false;
                }
                // target is non containment, and does not contain reference to moved object 
                if (target instanceof UniqueRule) {
                    if (SprayLayoutService.isCompartment(context.getTargetContainer())) {
                        ...
                        if ((id != null) && (id.equals("properties"))) {
                            if( ! ((UniqueRule)target).getProperties().contains(source) ){
                                return true;
                            }
                        }
                    }
            ...
            public void moveShape(IMoveShapeContext context) {
                ...
                if (target instanceof UniqueRule) {
                    if (SprayLayoutService.isCompartment(context.getTargetContainer())) {
                        String id = GraphitiProperties.get(context.getTargetContainer(), ISprayConstants.TEXT_ID);
                        if ((id != null) && (id.equals("properties"))) {
                            ((UniqueRule) target).getProperties().add((StringProperty) source);
                ...
                if (target instanceof BusinessClass) {
                    if (SprayLayoutService.isCompartment(targetContainer) ) {
                        ...
                        if ((id != null) && (id.equals("attributes"))) {
                            ...
                            ((BusinessClass) target).getProperties().add((StringProperty) source);
                            addContext.setNewObject(source);
                            addContext.setLocation(context.getX(), context.getX());
                            addContext.setTargetContainer(targetContainer);
                            getFeatureProvider().addIfPossible(addContext);
