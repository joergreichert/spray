...
public class SimpleUITest extends AbstractGraphitiTest {
    ...
    public void testCreateNewBusinessClass() throws Exception {
        ...
        SprayProjectWizardConfiguration wizardConfiguration = SprayProjectWizardConfiguration.init().firstPage().projectName(projectName).diagramTypeName(expectedDiagramTypeName).browseRegisteredEPackages(true).setGenModel(false).ePackageUriToBrowseInRegisteredEPackages("http://www.mod4j.org/busmod").ePackageUri("platform:/plugin/org.mod4j.dsl.businessdomain.mm/model/BusinessDomainDsl.ecore").genModelUri("platform:/plugin/org.mod4j.dsl.businessdomain.mm/model/BusinessDomainDsl.genmodel").selectType(true).modelEClassToSelect("BusinessDomainModel").modelEClass("BusinessDomainDsl.BusinessDomainModel").setFileExtension(false).fileExtension("businessdomaindsl").wizardConfiguration().secondPage().defaultBasePackage("org.eclipselabs.spray.examples.simpleuitest").wizardConfiguration();
        ...
        createBusinessClass(ged, objectFromToolbar, businessClassName, targetX, targetY);
        assertBusinessClassRepresentationExists(ged, businessClassName, targetX, targetY);
    }

    @Override
    protected void executeSprayWizard(SWTBotShell shell, SprayProjectWizardConfiguration wizardConfiguration) {
        SprayProjectWizardTestHelper wizardTestHelper = new SprayProjectWizardTestHelper(bot);
    ...
    private void createBusinessClass(final SWTBotGefEditor ged, final String objectFromToolbar, final String name, int targetX, int targetY) {
    ...
    private void assertBusinessClassRepresentationExists(SWTBotGefEditor ged, String shapeName, int targetX, int targetY) {
    ...
    private IFigure assertFigure(int targetX, int targetY, SWTBotGefEditPart editPart) {
    ...
    private void assertRoundedRectangle(int targetX, int targetY, IFigure figure) {
        assertTrue("rounded rectangle expected", figure instanceof GFRoundedRectangle);
        assertTrue("rounded rectangle should be a specialization of abstract shape", figure instanceof GFAbstractShape);
        GFRoundedRectangle rectangle = (GFRoundedRectangle) figure;
        try {
            Method method = GFAbstractShape.class.getDeclaredMethod("getGraphicsAlgorithm");
